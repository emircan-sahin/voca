import { useEffect } from 'react';
import { Card, CardContent } from 'poyraz-ui/atoms';
import { IUser } from '@voca/shared';
import { useAuthStore } from '~/stores/auth.store';
import { api } from '~/lib/axios';
import axiosInstance from '~/lib/axios';
import vocaLogo from '~/assets/voca_logo.png';

export const WelcomePage = () => {
  const { setAuth } = useAuthStore();

  useEffect(() => {
    const cleanup = window.electronAPI.auth.onAuthCallback(async ({ token, refreshToken }) => {
      // Temporarily set tokens so the interceptor can attach the Bearer header
      useAuthStore.setState({ token, refreshToken });

      try {
        const res = await api.get<IUser>('/auth/me');
        if (res.data) {
          setAuth({ user: res.data, token, refreshToken });
        }
      } catch {
        useAuthStore.setState({ token: null, refreshToken: null });
      }
    });

    return cleanup;
  }, [setAuth]);

  const handleGoogleLogin = () => {
    window.electronAPI.auth.openProvider(`${axiosInstance.defaults.baseURL}/auth/google`);
  };

  return (
    <div className="min-h-screen bg-[#fafafa] flex items-center justify-center p-6">
      <Card variant="bordered" className="w-full max-w-sm !border-dashed !border-[#e5e5e5]">
        <CardContent className="p-8">
          <div className="flex justify-center mb-6">
            <img src={vocaLogo} alt="Voca" className="w-16 h-16 rounded-full" />
          </div>

          <h1 className="text-xl font-bold text-[#171717] text-center mb-2">
            Welcome to Voca
          </h1>
          <p className="text-[#737373] text-sm text-center mb-6 leading-relaxed">
            Sign in to start recording and transcribing your audio.
          </p>

          <button
            onClick={handleGoogleLogin}
            className="w-full flex items-center justify-center gap-2 px-3 py-2 text-sm font-medium text-[#171717] bg-white border border-solid border-neutral-300 rounded-lg hover:bg-neutral-50 transition-colors"
          >
            <svg width="16" height="16" viewBox="0 0 24 24">
              <path
                d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
                fill="#4285F4"
              />
              <path
                d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                fill="#34A853"
              />
              <path
                d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                fill="#FBBC05"
              />
              <path
                d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                fill="#EA4335"
              />
            </svg>
            Sign in with Google
          </button>

          <p className="text-xs text-[#737373] text-center mt-6">Voca v0.0.1</p>
        </CardContent>
      </Card>
    </div>
  );
};
